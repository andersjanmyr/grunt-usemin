...
//
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath, prefix) {
    ...
    regexps.forEach(function (rxl) {
      ...
      content = content.replace(rxl[0], function (match, src) {
        ...
        var out = filterOut(file);
        if (prefix && out[0] === '/') {
          out = prefix + out;
        }
        var res = match.replace(src, out);
        if (srcfile !== file) {
...
FileProcessor.prototype.process = function(filename, assetSearchPath, prefix) {
  debug('processing file %s %s %s', filename, assetSearchPath, prefix);
  ...
  if (assetSearchPath && assetSearchPath.length !== 0) {
	...
	var content = this.replaceWithRevved(this.replaceBlocks(this.file), this.file.searchPath, prefix);
